import { ConversionResult, ParameterDisplay, ConverterConfig, createConverter } from "./general";
...
const TO_INTERMEDIATE_FACTORS: Record<string, (value: number, params: ParsedParams) => number> = {
    "rpm": (value, _params) => ((value * (2 * Math.PI)) / 60),
    "rps": (value, _params) => value * (2 * Math.PI),
    "rad/s": (value, _params) => value,
    "deg/s": (value, _params) => value * (Math.PI / 180),
};

const FROM_INTERMEDIATE_FACTORS: Record<string, (value: number, params: ParsedParams) => number> = {
    "rpm": (value, _params) => ((value * 60) / (2 * Math.PI)),
    "rps": (value, _params) => value / (2 * Math.PI),
    "rad/s": (value, _params) => value,
    "deg/s": (value, _params) => (value * 180) / Math.PI,
};
...
function parseAngularVelocityParameters(parameterStrings: string[]): ParsedParams {
...
const angularVelocityConfig: ConverterConfig = {
