...
const TORQUE_UNITS: string[] = ["nm", "n·m", "inlb", "in-lb", "lbft", "lbf·ft", "ozin", "oz-in", "n", "newton", "newtons", "lbf", "pound-force"];
...
const TO_INTERMEDIATE_FACTORS: Record<string, (value: number, params: ParsedParams) => number> = {
  "N·m": (value, _params) => value,
  "in·lb": (value, _params) => value / 8.850745767,
  "lb·ft": (value, _params) => value / 0.737562149,
  "oz·in": (value, _params) => value / 141.611932,
    ...
    return [
        {
            ...
            value: parsed.hasWheelRadius
                ...
                : `${(TORQUE_DEFAULTS.wheelRadius * 39.37).toFixed(1)}in`,
            isDefault: !parsed.hasWheelRadius,
        },
    ];
}

const torqueConfig: ConverterConfig = {
    name: "Torque",
    units: TORQUE_UNITS,
